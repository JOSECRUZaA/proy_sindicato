...
const AfiliadoPortal = () => {
  ...
  const fetchAffiliatePortalData = async () => {
    try {
      ...
      const results = await Promise.allSettled([
        supabase.from('vehiculos').select('*').eq('id_propietario', affData.id_perfil),
        supabase.from('chofer_vehiculo').select(`
          vehiculos ( id_vehiculo, numero_disco, placa, numero_linea, marca, modelo, estado )
        `).eq('id_chofer', affData.id_perfil).eq('estado', 1).is('fecha_fin', null),
        supabase.from('obligaciones_financieras').select(`
          id_obligacion, gestion, mes, monto_total, monto_pagado, estado, fecha_registro, concepto
        `).eq('id_afiliado', affData.id_perfil).eq('tipo_obligacion', 'Cuota').order('fecha_registro', { ascending: false }),
        supabase.from('obligaciones_financieras').select('*').eq('id_afiliado', affData.id_perfil).eq('tipo_obligacion', 'Multa').order('fecha_registro', { ascending: false })
      ]);

      const ownedData = results[0].status === 'fulfilled' && !results[0].value.error ? results[0].value.data : [];
      const driveData = results[1].status === 'fulfilled' && !results[1].value.error ? results[1].value.data : [];
      const cuotasData = results[2].status === 'fulfilled' && !results[2].value.error ? results[2].value.data : [];
      const multasData = results[3].status === 'fulfilled' && !results[3].value.error ? results[3].value.data : [];
  ...
  const debtCuotasAmount = pendingCuotas.reduce((sum, c) => sum + parseFloat(c.monto_total), 0);
  
  const pendingMultas = multas.filter(m => m.estado === 'Pendiente');
  const debtMultasAmount = pendingMultas.reduce((sum, m) => sum + parseFloat(m.monto_total), 0);
  ...
  return (
    <div className="animate-fade" style={{ padding: '0 0.5rem 3rem' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, var(--primary) 0%, #1e40af 100%)', borderRadius: '20px', padding: '2.5rem', color: 'white', marginBottom: '2.5rem', position: 'relative', overflow: 'hidden', boxShadow: 'var(--shadow-lg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', background: 'rgba(255, 255, 255, 0.1)', padding: '1rem 1.5rem', borderRadius: '16px', backdropFilter: 'blur(4px)', border: '1px solid rgba(255, 255, 255, 0.2)' }}>
          <img src="/logo-sindicato.jpg" alt="Logo Sindicato" style={{ width: '80px', height: '80px', borderRadius: '50%', objectFit: 'cover', border: '3px solid white', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }} />
          <span style={{ fontSize: '0.75rem', fontWeight: '700', letterSpacing: '0.05em', textTransform: 'uppercase', opacity: 0.9 }}>Sindicato 15 de Junio</span>
        </div>
      </div>
          ...
          <div style={{ background: 'white', border: '1px solid var(--border-light)', borderRadius: '20px', padding: '1.5rem', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ maxHeight: '250px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {cuotas.length === 0 ? (
                ...
                cuotas.map(c => (
                  <div key={c.id_obligacion} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', borderBottom: '1px solid var(--bg-subtle)' }}>
                    <div>
                      <div style={{ fontWeight: '600', color: 'var(--secondary)', fontSize: '0.9rem' }}>{c.concepto}</div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.1rem' }}>Mes: {c.mes}/{c.gestion}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <span style={{ fontWeight: '700', color: 'var(--secondary)', fontSize: '0.9rem', display: 'block' }}>Bs. {c.monto_total}</span>
                      <span className={`badge ${c.estado === 'Pagado' ? 'badge-success' : 'badge-warning'}`} style={{ fontSize: '0.65rem', padding: '0.1rem 0.5rem', marginTop: '0.2rem' }}>
                        {c.estado}
                      </span>
                    </div>
                  </div>
          ...
          <div style={{ background: 'white', border: '1px solid var(--border-light)', borderRadius: '20px', padding: '1.5rem', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', maxHeight: '420px', overflowY: 'auto' }}>
              {multas.length === 0 ? (
                ...
                multas.map(m => (
                  <div key={m.id_obligacion} style={{ padding: '1rem', background: m.estado === 'Pendiente' ? '#fffbeb' : 'var(--bg-subtle)', border: m.estado === 'Pendiente' ? '1px solid #fee2e2' : '1px solid var(--border-light)', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ flex: 1, marginRight: '0.5rem' }}>
                      <div style={{ fontWeight: '700', color: 'var(--secondary)', fontSize: '0.9rem' }}>{m.concepto}</div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.15rem' }}>Fecha emisión: {m.fecha_registro}</div>
                      {m.observacion && (
                    ...
                    <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                      ...
                      <span className={`badge ${m.estado === 'Pagado' ? 'badge-success' : m.estado === 'Condenado' ? 'badge-secondary' : 'badge-danger'}`} style={{ fontSize: '0.65rem', padding: '0.1rem 0.5rem', marginTop: '0.4rem' }}>
                        {m.estado}
                      </span>
                    </div>
                  </div>
          ...
          <div style={{ background: 'white', border: '1px solid var(--border-light)', borderRadius: '20px', padding: '1.5rem', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <span>Nombres: <strong style={{ color: 'var(--secondary)' }}>{affiliate?.nombres} {affiliate?.paterno} {affiliate?.materno}</strong></span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <FileText size={16} color="var(--primary)" />
                <span>Cédula: <strong style={{ color: 'var(--secondary)' }}>{affiliate?.ci}</strong></span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Phone size={16} color="var(--primary)" />
                <span>Celular: <strong style={{ color: 'var(--secondary)' }}>{affiliate?.celular || 'No registrado'}</strong></span>
              </div>
            </div>
          </div>
